...
static PyGetSetDef getset[] = {
	{
		const_cast<char*>("__dict__"), pybind11::detail::pybind11_get_dict, pybind11::detail::pybind11_set_dict,
		nullptr, nullptr
	},
...
static void* get_object_ptr(const void* ptr)
{
	...
	for (it = pybind11::detail::get_internals().registered_instances.begin(); it != pybind11::detail::get_internals()
	                                                                                .registered_instances.end(); ++it)
...
{
	static const char* keywordList[] = {"key", "value", nullptr};
	int key;
	PyObject* value;
	...
	void* data_addr = (void *)(PyLong_AsUnsignedLong(PyObject_GetAttrString(self, "__tarray_data_addr__")) + key *
		obj_size);
	...
	namespace detail
	{
		...
		struct tarray_pointer_caster
		{
			...
			bool load(handle src, bool convert)
			{
				...
				value.Data = (Value *)((tMalloc)BL2SDK::pGMalloc[0]->VfTable[1])(
					BL2SDK::pGMalloc[0], sizeof(Value) * s.size(), 8);
				value.Count = s.size();
				value.Max = s.size();
				int x = 0;
				for (auto it : s)
			...
			static handle cast(T&& src, return_value_policy policy, handle parent)
			{
				...
				for (size_t index = 0; index < src.Count; index++)
				{
					...
					if (index < src.Count)
					{
						...
						value_ = reinterpret_steal<object
				...
				if (std::is_pointer<Value>::value)
					is_ptr = Py_True;
				PyObject_GenericSetAttr((PyObject *)o, PyUnicode_FromString("__tarray_data_addr__"),
				                        PyLong_FromUnsignedLong((unsigned long)src.Data));
				PyObject_GenericSetAttr((PyObject *)o, PyUnicode_FromString("__tarray_obj_size__"),
				                        PyLong_FromUnsignedLong((unsigned long)sizeof(Value)));
				PyObject_GenericSetAttr((PyObject *)o, PyUnicode_FromString("__tarray_count__"),
				                        PyLong_FromUnsignedLong((unsigned long)src.Count));
				PyObject_GenericSetAttr((PyObject *)o, PyUnicode_FromString("__tarray_max__"),
				                        PyLong_FromUnsignedLong((unsigned long)src.Max));
		...
		struct type_caster<TArray<Type>> : tarray_pointer_caster<TArray<Type>, Type>
		{
		};
	}
}

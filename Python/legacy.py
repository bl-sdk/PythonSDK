...
class LegacyMod(bl2sdk.BL2MOD):
	...
	def Populate():
		...
		for filename in os.listdir(LegacyMod.BinariesDirectory):
			...
			if mod == None:
				...
				if not os.path.isfile(os.path.join(LegacyMod.BinariesDirectory, filename)):
					continue
		...
		for mod in conflictedMods:
			...
			conflicts = [conflictingMod.Name for conflictingMod in (mod.Conflicts - {mod})]
			# Update the mod's description with a warning message that includes
			# the list of mod names.
			mod.Description += "\n\nWARNING: This may conflict with the following mods: " + ", ".join(conflicts)
	...
	SetPattern = re.compile(r"\s*set\s+(\S+)\s+(\w+)\S*\s+(?:\((.+)\))?\s*", re.IGNORECASE)
	...
	def Update(self):
		...
		self.Description = "Mod file found at: %s\n\nUse BLCMM to edit options for this mod." % self.Filepath
		...
		with open(self.Filepath, 'r') as file:
			...
			for key, value in zip(hotfixKeys, hotfixValues):
				...
				hotfixPattern = LegacyMod.PatchHotfixPattern if key.startswith('SparkPatchEntry') else LegacyMod.HotfixPattern
	...
	def Execute(self):
		...
		micropatch = None
		for service in bl2sdk.FindObject("GearboxAccountData", "Transient.GearboxAccountData_1").Services:

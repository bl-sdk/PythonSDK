...
void CLuaInterface::SetPaths()
{
	...
	m_luaPath = Util::Narrow(Settings::GetBinFile(L"..\\plugins"));
	Logging::LogF("[Lua] Lua Path = %s\n", m_luaPath.c_str());

	lua_getfield(m_pState, LUA_GLOBALSINDEX, LUA_LOADLIBNAME);

	std::string newPath = Util::Format("%s\\include\\modules\\?.lua;%s\\include\\modules\\?\\init.lua;.\\?.lua;", m_luaPath.c_str(), m_luaPath.c_str());

...
void CLua::CleanupState()
{
	lua_close(m_pState);
}

LuaStatus CLua::InitializeModules()
{
	m_modulesInitialized = false;
...
bool CLua::CheckHash(FileHash* fileHash)
{
...
int CLua::RunString(const char* string)
{
...
int CLua::DoFile(const std::string& filename)
{
	return DoFileAbsolute(Util::Format("%s\\%s", m_luaPath.c_str(), filename.c_str()));
}

int CLua::DoFileAbsolute(const std::string& path)
{
	return dofile(m_pState, path.c_str());
}

void CLua::CallShutdownFuncs()
{
